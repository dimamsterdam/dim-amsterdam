        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          >
            <div className="flex flex-wrap gap-2 mb-6">
              {heroContent.tags.map((tag, index) => (
                <Link
                  key={index}
                  to={tag === "Organisatieontwikkeling" ? "/diensten/organisatieontwikkeling" : tag === "Leiderschaps- & Teamontwikkeling" ? "/diensten/leiderschapsontwikkeling" : "/diensten/executive-coaching"}
            ...
            <h1 className="text-4xl md:text-5xl font-display font-bold tracking-tight text-gray-700 leading-tight">
              {heroContent.heading.split(' ').map((word, i) => (
                word.toLowerCase() === 'impact' || word.toLowerCase() === 'werkplezier' 
                  ? <span key={i} className="text-[#F97316]">{word} </span> 
                  : word + ' '
              ))}
            </h1>
          ...
          >
            <div className="aspect-video rounded-lg overflow-hidden shadow-sm border border-gray-100">
              <img src={heroContent.heroImage} alt="Professional collaboration" className="w-full h-full object-cover" />
            </div>
          </motion.div>
        </div>
      </div>
