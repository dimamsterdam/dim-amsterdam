  ...
  const handleStepClick = (index: number) => {
    ...
    if (window.innerWidth < 768 && timelineRef.current) {
      ...
      if (stepElements[index]) {
        timelineRef.current.scrollLeft = stepElements[index].getBoundingClientRect().left + 
          timelineRef.current.scrollLeft - timelineRef.current.getBoundingClientRect().left - 
          window.innerWidth / 2 + stepElements[index].getBoundingClientRect().width / 2;
      }
    }
  };
    ...
    <section className="section-padding bg-background relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 md:px-6">
        ...
        <div className="mb-16">
          <div 
            ref={timelineRef}
            className="flex space-x-4 md:space-x-0 md:justify-between overflow-x-auto md:overflow-visible py-6 md:px-8 -mx-4 md:mx-0 scrollbar-hide"
          >
            {approachSteps.map((step, index) => (
              <button
                key={index}
                onClick={() => handleStepClick(index)}
                className={`timeline-step flex flex-col items-center flex-shrink-0 w-28 md:w-auto px-2 transition-all duration-300 ${
                  selectedIndex === index ? 'opacity-100' : 'opacity-70 hover:opacity-100'
                }`}
              >
                <div className={`relative z-10 w-14 h-14 flex items-center justify-center rounded-full mb-3 text-lg transition-all duration-300 ${
                  selectedIndex === index 
                ...
                <p className={`font-medium text-sm md:text-base transition-colors duration-300 ${
        ...
        <AnimatedSection 
          key={selectedIndex}
          className="max-w-3xl mx-auto bg-accent/30 rounded-xl p-8 md:p-10 border border-accent"
          animation="scale-in"
        >
