  ...
  return <section className="section-padding bg-accent/30">
      <div className="container mx-auto px-4 md:px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
          {services.map((service, index) => <AnimatedSection key={index} delay={index * 0.1} animation="slide-from-right" className="h-full">
              <Link to={service.link} className="group block h-full" onMouseEnter={() => setHoverIndex(index)} onMouseLeave={() => setHoverIndex(null)}>
                <div className={`h-full rounded-xl p-8 transition-all duration-300 border ${hoverIndex === index ? 'border-primary shadow-lg translate-y-[-4px]' : 'border-border hover:border-primary/50'}`}>
